...
const spaces = '                                                                 ';
const old1 = spaces + 'const isCardColumn = (colName = "") => (colName || "").toLowerCase().includes("card album");';
const new1 = [
  '  const isCardColumn = (colName = "") => (colName || "").toLowerCase().includes("card album");',
  '  const isLDColumn = (colName = "") => (colName || "").toLowerCase().includes("light") || (colName || "").toLowerCase().includes("dark") || (colName || "").toLowerCase().includes("lnd");',
].join('\n');

if (content.includes(old1)) {
  content = content.replace(old1, new1);
  console.log('Fix 1 applied: isCardColumn fixed, isLDColumn added');
} else {
  console.log('Fix 1 NOT FOUND - checking trimmed...');
  // Try with different whitespace
  const trimmedSearch = 'const isCardColumn = (colName = "") => (colName || "").toLowerCase().includes("card album");';
...
// ── Fix 2: Replace entire old tracker block ───────────────────────────────────
...
console.log('start_idx:', startIdx, 'end_idx:', endIdx);

if (startIdx !== -1 && endIdx !== -1) {
  const newTracker = `            {/* 2. Global World Master Tracker (Cards + LnD Tabs) */}
  ...
  content = content.slice(0, startIdx) + newTracker + content.slice(endIdx + endMarker.length);
  console.log('Fix 2 applied: World Master Tracker inserted');
} else {
