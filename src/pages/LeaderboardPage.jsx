    ...
    <div>
      <div className="page-header">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="page-title">ðŸ† Leaderboard</h1>
            <p className="page-subtitle">Rankings based on scoring formula â€” auto-computed</p>
          </div>
          <div className="flex gap-2">
            <button className={`btn ${lbMode==="gl"?"btn-primary":"btn-ghost"}`} onClick={() => setLbMode("gl")}>âš” Guild League</button>
            <button className={`btn ${lbMode==="eo"?"btn-primary":"btn-ghost"}`} onClick={() => setLbMode("eo")}>ðŸ° Emperium Overrun</button>
          </div>
        </div>
      </div>
                ...
                <tr key={m.memberId}>
                  <td>
                    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,minWidth:40}}>
                      {m.rank <= 3 && <span style={{fontSize:18}}>{["ðŸ¥‡","ðŸ¥ˆ","ðŸ¥‰"][m.rank-1]}</span>}
      ...
      {lbMode === "eo" && (
        <div className="card" style={{marginBottom:16}}>
          <div className="section-header">
            <div className="font-cinzel text-xs text-muted" style={{letterSpacing:2,textTransform:"uppercase"}}>ðŸ° Emperium Overrun Rankings</div>
            <div className="text-xs text-muted">{eoEvents.length} EO events Â· Star ratings by officers</div>
          </div>
          {eoLb.every(m => m.totalEoScore === 0) ? (
            <div className="empty-state">
              <div className="empty-state-icon">â­</div>
              <div className="empty-state-text">No EO ratings yet â€” go to Events tab and rate members on EO events</div>
            </div>
          ) : (
            <div className="table-wrap">
              <table>
                ...
                <tbody>
                  {eoLb.map((m) => (
                    <tr key={m.memberId}>
                      <td>
                        <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,minWidth:40}}>
                          {m.eoRank <= 3 && <span style={{fontSize:18}}>{["ðŸ¥‡","ðŸ¥ˆ","ðŸ¥‰"][m.eoRank-1]}</span>}
                      ...
                      <td>
                        <div style={{display:"flex",gap:2,alignItems:"center"}}>
                          {[1,2,3,4,5].map(s => (
                            <span key={s} style={{fontSize:14,color:s<=Math.round(m.avgRating)?"var(--gold)":"rgba(99,130,230,0.2)"}}>â˜…</span>
                          ))}
      ...
      {lbMode === "gl" && <div className="card mt-2" style={{marginTop:16}}>
        ...
        <div style={{marginBottom:14}}>
          <div style={{fontSize:11,letterSpacing:2,textTransform:"uppercase",color:"var(--text-muted)",fontWeight:700,marginBottom:10}}>âš” Score Classification</div>
          <div className="flex gap-4" style={{flexWrap:"wrap"}}>
            {[["Core","badge-core","Score > 80"],["Active","badge-active","Score 60â€“80"],["Casual","badge-casual","Score 40â€“60"],["At Risk","badge-atrisk","Score < 40"]].map(([k,c,d])=>(
        ...
        <div>
          <div style={{fontSize:11,letterSpacing:2,textTransform:"uppercase",color:"var(--text-muted)",fontWeight:700,marginBottom:10}}>ðŸŽ¯ Attendance Classification</div>
          <div className="flex gap-4" style={{flexWrap:"wrap"}}>
            {[
              ["âœ… Reliable","badge-active","80%+ attendance","Dependable guild member"],
              ["âš  Average","badge-casual","60â€“79% attendance","Monitor closely"],
              ["ðŸš¨ At Risk","badge-atrisk","Below 60%","Needs follow-up"],
            ].map(([k,c,d,desc])=>(
              <div key={k} className="flex items-center gap-2">
                <span className={`badge ${c}`}>{k}</span>
                <div>
