  ...
  return (
    <div>
      <div className="page-header">
        <h1 className="page-title">ðŸ“¥ Import CSV</h1>
        <p className="page-subtitle">I-import ang Members list mula sa Google Sheets</p>
      </div>
      <div className="card" style={{marginBottom:20}}>
        <div className="card-title">ðŸ“‹ Paano mag-export mula sa Google Sheets</div>
        <div style={{fontSize:13,color:"var(--text-secondary)",lineHeight:1.8}}>
          <div>1. Buksan ang Google Sheets â†’ Members sheet</div>
          <div>2. <strong>File â†’ Download â†’ Comma Separated Values (.csv)</strong></div>
      ...
      <div className="card" style={{marginBottom:20}}>
        <div className="card-title">ðŸ“ Upload CSV File</div>
        ...
        {error && <div style={{marginTop:12,padding:"10px 14px",background:"rgba(224,80,80,0.1)",border:"1px solid rgba(224,80,80,0.3)",borderRadius:8,color:"var(--red)",fontSize:13}}>âš ï¸ {error}</div>}
        {imported && <div style={{marginTop:12,padding:"10px 14px",background:"rgba(64,201,122,0.1)",border:"1px solid rgba(64,201,122,0.3)",borderRadius:8,color:"var(--green)",fontSize:13}}>âœ… Import successful! Pumunta sa Members tab para makita.</div>}
      </div>
      {preview.length > 0 && (
        <div className="card">
          <div className="flex items-center justify-between mb-4">
            <div>
              <div className="card-title" style={{marginBottom:4}}>ðŸ‘ Preview â€” {preview.length} members found</div>
              <div className="text-xs text-muted">I-check kung tama ang data bago i-confirm ang import</div>
            </div>
            <div className="flex gap-2">
              <button className="btn btn-ghost" onClick={() => confirmImport("merge")}>ðŸ”€ Merge</button>
              <button className="btn btn-primary" onClick={() => confirmImport("replace")}>â™»ï¸ Replace All</button>
            </div>
          </div>
          ...
          <div style={{marginTop:12,padding:"10px 14px",background:"rgba(240,192,64,0.06)",border:"1px solid rgba(240,192,64,0.2)",borderRadius:8,fontSize:12,color:"var(--text-muted)"}}>
            ðŸ’¡ <strong style={{color:"var(--gold)"}}>Merge</strong> â€” mag-a-update ng existing at magda-dagdag ng bago. Hindi mabubura ang current data.<br/>
            ðŸ’¡ <strong style={{color:"var(--accent)"}}>Replace All</strong> â€” mabubura lahat at papalitan ng CSV data.
          </div>
        </div>
      )}
    </div>
