  ...
  return (
    <div>
      <div className="page-header">
        ...
        <p className="page-subtitle">Manage officer accounts â€” Admin only</p>
      </div>
        ...
        {showForm && (
          <div style={{background:"rgba(99,130,230,0.05)",border:"1px solid var(--border)",borderRadius:10,padding:16,marginBottom:16}}>
            ...
            <div style={{background:"rgba(240,192,64,0.06)",border:"1px solid rgba(240,192,64,0.2)",borderRadius:8,padding:"8px 12px",fontSize:12,color:"var(--text-muted)",marginBottom:12}}>
              âš  After creating, share the email and password with the officer. They can change their password later.
            </div>
            ...
            <tbody>
              ...
              {users.map((u, i) => (
                <tr key={u.uid || i}>
                  ...
                  <td><span className={`badge ${u.role==="admin"?"badge-core":"badge-support"}`}>{u.role==="admin"?"â­ Admin":"ðŸ›¡ Officer"}</span></td>
                  <td className="text-muted" style={{fontSize:12}}>{u.createdAt ? new Date(u.createdAt).toLocaleDateString() : "â€”"}</td>
                </tr>
              ))}
            </tbody>
          </table>
