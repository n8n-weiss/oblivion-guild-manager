...
function WeeklyReportPage() {
  ...
  const dateLabel = filteredEvents.length > 0
    ? `${filteredEvents[0].eventDate} â€“ ${filteredEvents[filteredEvents.length-1].eventDate}`
    : "No events in this period";

  // Discord text
  const discordText = `âš” **OBLIVION WEEKLY SUMMARY** | ${dateLabel}
â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”
ðŸ“… Events: ${filteredEvents.length} (${glEvents.length} GL Â· ${eoEvents.length} EO)
âœ… Attendance: ${attRate}% (${presentSlots}/${totalSlots})
âŒ Absences: ${totalAbsences}

ðŸ† TOP PERFORMERS
${topPerformers.map((p, i) => `${["ðŸ¥‡","ðŸ¥ˆ","ðŸ¥‰"][i]} ${p.ign} â€” ${p.score}pts (${p.attPct}% att)`).join("\n")}${needsAttention.length > 0 ? `

âš  NEEDS ATTENTION
${needsAttention.slice(0,3).map(m => `${m.consecutiveAbsent >= 3 ? "ðŸ”´" : "ðŸŸ¡"} ${m.ign} â€” ${m.consecutiveAbsent >= 2 ? `${m.consecutiveAbsent} consecutive absences` : `${m.absentCount} absences`} (${m.attPct}% att)`).join("\n")}` : ""}

â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”
ðŸ“Š Full rankings: https://n8n-weiss.github.io/oblivion-guild-manager/`;
  ...
  return (
    <div>
      <div className="page-header">
        <h1 className="page-title">ðŸ“‹ Weekly Report</h1>
        <p className="page-subtitle">Auto-generated summary â€” ready to copy paste sa Discord</p>
      </div>
      ...
      <div className="card" style={{marginBottom:20}}>
        ...
        {filteredEvents.length === 0 && (
          <div className="text-muted text-sm" style={{marginTop:12,padding:"10px 14px",background:"rgba(224,80,80,0.05)",borderRadius:8,border:"1px solid rgba(224,80,80,0.2)"}}>
            âš  No events found for this period.
          </div>
        )}
      </div>
        ...
        <>
          ...
          <div className="stats-grid" style={{gridTemplateColumns:"repeat(4,1fr)"}}>
            <div className="stat-card" style={{"--stat-accent":"var(--accent)"}}>
              ...
              <div className="stat-change">{glEvents.length} GL Â· {eoEvents.length} EO</div>
            </div>
            ...
            <div className="stat-card" style={{"--stat-accent":"var(--gold)"}}>
              ...
              <div className="stat-change">{topScorer?.ign || "â€”"}</div>
            </div>
            <div className="stat-card" style={{"--stat-accent":"var(--red)"}}>
              <div className="stat-icon">âŒ</div>
          ...
          <div className="grid-2" style={{marginBottom:20}}>
            {/* Top Performers */}
            <div className="card">
              <div className="card-title">ðŸ† Top Performers</div>
              {topPerformers.length === 0 ? (
                ...
                topPerformers.map((p, i) => (
                  <div key={p.memberId} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 0",borderBottom: i < topPerformers.length-1 ? "1px solid var(--border)" : "none"}}>
                    <span style={{fontSize:22}}>{["ðŸ¥‡","ðŸ¥ˆ","ðŸ¥‰"][i]}</span>
            ...
            {/* Needs Attention */}
            <div className="card">
              <div className="card-title">âš  Needs Attention</div>
              {needsAttention.length === 0 ? (
                ...
                needsAttention.slice(0, 5).map((m, i) => (
                  <div key={m.memberId} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 0",borderBottom: i < Math.min(needsAttention.length,5)-1 ? "1px solid var(--border)" : "none"}}>
                    <span style={{fontSize:18}}>{m.consecutiveAbsent >= 3 || m.attPct < 40 ? "ðŸ”´" : "ðŸŸ¡"}</span>
          ...
          <div className="card">
            <div className="flex items-center justify-between mb-3">
              <div>
                <div className="card-title" style={{marginBottom:2}}>ðŸ“¢ Discord-Ready Text</div>
              ...
              <button className={`btn ${copied ? "btn-primary" : "btn-ghost"}`} onClick={copyToClipboard}>
                ...
                {copied ? "Copied! âœ“" : "Copy for Discord"}
              </button>
            </div>
            <div style={{
