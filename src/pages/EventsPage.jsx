...
function EventsPage() {
  ...
  const deleteEvent = (eventId) => {
    ...
    writeAuditLog(currentUser?.email, currentUser?.displayName || currentUser?.email, "event_delete", `Deleted event ${ev?.eventType} â€” ${ev?.eventDate}`);
  };
  ...
  const createEvent = () => {
    ...
    writeAuditLog(currentUser?.email, currentUser?.displayName || currentUser?.email, "event_create", `Created ${form.eventType} event â€” ${form.eventDate}`);
    setShowModal(false);
  };
  ...
  const toggleAtt = (memberId, eventId) => {
    ...
    writeAuditLog(currentUser?.email, currentUser?.displayName || currentUser?.email, "attendance_toggle", `Marked ${member?.ign} as ${newStatus} â€” ${ev?.eventType} ${ev?.eventDate}`);
  };
  ...
  const savePerformance = (memberId, eventId) => {
    ...
    writeAuditLog(currentUser?.email, currentUser?.displayName || currentUser?.email, "score_save", `Saved scores for ${member?.ign} â€” CTF: ${edits.ctf1 ?? 0}+${edits.ctf2 ?? 0}+${edits.ctf3 ?? 0}=${ctfTot}, Perf: ${edits.performancePoints ?? 0} (${ev?.eventDate})`);
  };
  ...
  return (
    <div>
      <div className="page-header">
        <h1 className="page-title">ðŸ“… Events</h1>
                          ...
                          {selectedEvent.eventType === "Emperium Overrun" && (
                            <td>
                              {m.att?.status === "present" ? (
                                    ...
                                    return (
                                      <span key={star}
                                        onClick={() => {
                                          ...
                                          if (newRating > 0) writeAuditLog(currentUser?.email, currentUser?.displayName || currentUser?.email, "eo_rating", `Rated ${m.ign} ${newRating}/5 stars â€” EO ${selectedEvent.eventDate}`);
                                        }}
                                        style={{
                                ...
                                <span className="text-xs text-muted">â€” absent</span>
                              )}
                            </td>
                          )}
                        </tr>
      ...
      {showModal && (
        <Modal title="Create New Event" onClose={() => setShowModal(false)}
          ...
          <div className="text-xs text-muted mt-2" style={{padding:"10px 14px",background:"rgba(99,130,230,0.05)",borderRadius:8,border:"1px solid var(--border)"}}>
            â„¹ All {members.length} members will be auto-loaded as Present. Absence records will override to Absent.
          </div>
        </Modal>
      )}
    </div>
